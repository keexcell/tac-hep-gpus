...
__global__ void matrix_mult(const int *A, const int *B, int *C, int size) {
    ...
    for (int i = 0; i < (size+BLOCK_SIZE-1)/BLOCK_SIZE; i++){
		...
		if (i*BLOCK_SIZE + threadIdx.x < size && idy < size){
        	tempA[threadIdx.y][threadIdx.x] = A[idy*size + (i*BLOCK_SIZE+threadIdx.x)];
		}
		if (i*BLOCK_SIZE + threadIdx.y < size && idx < size){
        	tempB[threadIdx.y][threadIdx.x] = B[(i*BLOCK_SIZE+threadIdx.y)*size + threadIdx.x];
		}
		else{
		...
		for (int k = 0; k < BLOCK_SIZE; k++){
			temp_sum += tempA[threadIdx.y][k]*tempB[k][threadIdx.x];
